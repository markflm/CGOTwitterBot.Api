...
app.use((req, res, next) => {
	//valid api key before doing anything else with request
...
app.post('/user/:user/addteams', (req, res, next) => {
	...
	if (teamsValidated == 0) {
		...
		//only requests with newly validated teams should end up here.
...
app.post('/user/:user/removeteams', (req, res, next) => {
	...
	if (teamsToRemove === null) {
		res.status(400).json({
			message:
				'null provided as value to teamsToRemove key - invalid, request rejected with no deletions for user: ' +
					userName ?? 'nulluser',
		});
		return;
	}
